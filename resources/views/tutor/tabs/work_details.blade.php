        ...
        <tbody class="bg-white divide-y divide-gray-200" id="tutorTableBody">
            ...
            @forelse($rows as $assignment)
                @php
                    ...
                    $tutorName =
                        Auth::guard('tutor')->user()->full_name ?? (Auth::guard('tutor')->user()->username ?? 'N/A');

                    // Start and End Time
                    $startTime =
                        $workDetail && $workDetail->start_time
                            ? \Carbon\Carbon::parse($workDetail->start_time)->format('g:i A')
                            : ($schedule
                    ...
                    if ($workDetail && $workDetail->end_time) {
                        ...
                        $endTime = \Carbon\Carbon::parse($schedule->time)
                    ...
                    $rate =
                        $workDetail && $workDetail->rate_per_class
                            ? $workDetail->rate_per_class
                            : $schedule->rate ?? 'N/A';
                @endphp
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                        @php
                            ...
                            if ($assignment->class_status === 'pending_acceptance') {
                                ...
                                $hasSubmitted &&
                                in_array(strtolower($workDetail->status ?? ''), ['rejected', 'reject'])
                            ) {
                                $statusText = 'Rejected';
                                $dotColor = 'bg-red-400';
                            } elseif ($hasSubmitted) {
                                ...
                                $assignment->class_status === 'fully_assigned' ||
                        ...
                        <span class="inline-flex items-center gap-2">
                            ...
                            <!-- Status text -->
                            <span class="text-gray-800 text-xs font-medium">{{ $statusText }}</span>
                        </span>
                    </td>
            ...
            @empty
                <tr id="noResultsRow">
                    <td colspan="5" class="px-6 py-8 text-center text-gray-500">
                        ...
                        <p class="text-sm">You will see your class schedule here once a supervisor assigns you as main
                            tutor</p>
                    </td>
                </tr>
            @endforelse
...
<div id="tutorWorkDetailModal"
    ...
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-3xl mx-4 max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div class="bg-[#0E335D] text-white px-6 py-4 flex items-center justify-between flex-shrink-0">
            ...
            <button type="button" onclick="closeTutorWorkDetailModal()"
        ...
        <div class="overflow-y-auto flex-grow">
            <div class="p-6">
                ...
                <div
                    class="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-lg p-5 mb-6 border border-blue-200 shadow-sm">
                ...
                <div
                    class="bg-gradient-to-br from-green-50 to-emerald-50 rounded-lg p-5 mb-6 border border-green-200 shadow-sm">
                    <h3 class="text-lg font-semibold text-gray-800 mb-4 flex items-center">
                ...
                <div id="modal-rejection-notice" class="bg-red-50 border border-red-300 rounded-lg p-4 mb-4"
                    style="display: none;">
...
<div id="viewWorkDetailModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50"
    style="display: none;">
    <div class="bg-white rounded-lg shadow-lg max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div class="sticky top-0 bg-gray-50 border-b border-gray-200 px-6 py-4 flex items-center justify-between">
            ...
            <button onclick="document.getElementById('viewWorkDetailModal').style.display = 'none'"
        ...
        <div class="p-6 space-y-6">
            ...
            <div>
                <h3 class="text-sm font-semibold text-gray-900 mb-4 pb-2 border-b border-gray-200">Actual Times
                    Submitted</h3>
                <div class="grid grid-cols-2 gap-4">
                    <div>
            ...
            <div>
                ...
                <img id="vwd_proof_image" src="" alt="Proof of work"
                    class="w-full rounded border border-gray-200 mb-2"
                    style="max-height: 300px; object-fit: contain; display: none;">
                <a id="vwd_proof_link" href="" target="_blank"
                    class="text-blue-600 hover:text-blue-800 text-sm" style="display: none;">
            ...
            <div id="vwd_approval_section" style="display: none;">
                <h3 class="text-sm font-semibold text-gray-900 mb-4 pb-2 border-b border-green-200 text-green-700">
