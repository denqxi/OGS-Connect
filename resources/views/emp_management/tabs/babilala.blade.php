...
<div class="px-4 md:px-6 pt-4 md:pt-6 pb-3 border-b border-gray-200">
    <form method="GET" action="{{ route('employees.index') }}" id="babilalaFilterForm">
        ...
        <div class="flex items-center justify-between gap-x-6">
            ...
            <div class="flex items-center gap-x-4 flex-1">
                ...
                <!-- Status -->
                <select name="status" id="filterBabilalaStatus"
                        ...
                        onchange="handleBabilalaFilterChange('status')">
            ...
            <div class="flex items-center gap-x-4">
                ...
                <!-- Time Slot -->
                <select name="time_slot" id="filterBabilalaTimeSlot"
                        class="border border-gray-300 rounded-lg px-3 py-2 text-sm text-gray-700 bg-white
                               focus:border-[#2A5382] focus:ring-1 focus:ring-[#2A5382]/50"
                        onchange="handleBabilalaFilterChange('time_slot')">
                ...
                <!-- Day -->
                <select name="day" id="filterBabilalaDay"
                        class="border border-gray-300 rounded-lg px-3 py-2 text-sm text-gray-700 bg-white
                               focus:border-[#2A5382] focus:ring-1 focus:ring-[#2A5382]/50"
                        onchange="handleBabilalaFilterChange('day')">
    ...
    <table class="min-w-full divide-y divide-gray-200">
        <thead class="bg-gray-50 border-b border-gray-200">
            <tr>
                ...
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100" onclick="window.location.href='{{ route('employees.index', array_merge(request()->all(), ['tab' => 'babilala', 'sort' => request('sort') === 'name' && request('direction') === 'desc' ? '' : 'name', 'direction' => request('sort') === 'name' ? (request('direction') === 'asc' ? 'desc' : '') : 'asc'])) }}'">
...
<script>
    window.babilalaTotalResults = @json(isset($tutors) && method_exists($tutors, 'total') ? $tutors->total() : 0);
    
    function handleBabilalaFilterChange(changed) {
        // Time slot and day are mutually exclusive, but status works with both
        if (changed === 'time_slot') {
            document.getElementById('filterBabilalaDay').value = '';
        } else if (changed === 'day') {
            document.getElementById('filterBabilalaTimeSlot').value = '';
        }
        
        // Submit the form to apply filters
        document.getElementById('babilalaFilterForm').submit();
    }

    // Handle search input - submit form on Enter
    document.addEventListener('DOMContentLoaded', function() {
        const searchInput = document.getElementById('babilalaSearch');
