...
<div class="p-6 border-b border-gray-200">
    ...
    <form method="GET" action="{{ route('employees.index') }}" class="flex justify-between items-center space-x-4">
        ...
        <div class="flex items-center space-x-4">
            ...
            <button type="submit" class="px-4 py-2 bg-[#128AD4] text-white rounded-md text-sm hover:bg-[#128AD4]/80">
                Search
            </button>
        </div>
    </form>
    ...
    <table class="w-full">
        ...
        <tbody class="bg-white divide-y divide-gray-200" id="talk915TableBody">
            @forelse($tutors as $tutor)
                @php
                    $talk915Account = $tutor->accounts->where('account_name', 'Talk915')->first();
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        @if($talk915Account)
                            <div class="flex flex-col">
                                <span class="font-medium text-gray-700">{{ $talk915Account->getFormattedAvailableTimeAttribute() }}</span>
                                <span class="text-xs text-blue-600 font-medium">(Talk915 Account)</span>
                            </div>
                        @else
                            <span class="text-red-500 text-sm">No Talk915 availability</span>
                        @endif
                    </td>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                        <div class="flex space-x-2">
                            ...
                            @if($tutor->status === 'active')
                                <button onclick="toggleTutorStatus('{{ $tutor->tutorID }}', 'inactive')" 
                            ...
                            @else
                                <button onclick="toggleTutorStatus('{{ $tutor->tutorID }}', 'active')" 
                                        class="px-3 py-1 bg-green-100 text-green-600 rounded hover:bg-green-200 transition-colors text-xs font-medium" 
            ...
            @empty
                <tr>
                    <td colspan="5" class="px-6 py-4 text-center text-sm text-gray-500">
                        No tutors found with Talk915 accounts.
                    </td>
                </tr>
            @endforelse
        </tbody>
