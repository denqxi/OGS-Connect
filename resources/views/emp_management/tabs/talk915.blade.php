...
<div class="px-4 md:px-6 pt-4 md:pt-6 pb-3 border-b border-gray-200">
    <h2 class="text-xl font-bold text-[#FF4500]">Talk915 Account</h2>
</div>
...
<div class="px-4 md:px-6 py-3 border-b border-gray-200">
    ...
    <form method="GET" action="{{ route('employees.index') }}" id="talk915FilterForm" class="flex justify-between items-center space-x-4">
        ...
        <div class="flex items-center space-x-4">
            ...
            <select name="time_slot" onchange="document.getElementById('talk915FilterForm').submit()" class="border border-gray-300 rounded-md px-3 py-2 text-sm text-gray-600 bg-white">
            ...
            <select name="day" id="filterTalk915Day" class="border border-gray-300 rounded-md px-3 py-2 text-sm text-gray-600 bg-white" onchange="document.getElementById('talk915FilterForm').submit()">
    ...
    <table class="min-w-full divide-y divide-gray-200">
        <thead class="bg-gray-50 border-b border-gray-200">
            <tr>
                ...
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100" onclick="window.location.href='{{ route('employees.index', array_merge(request()->all(), ['tab' => 'talk915', 'sort' => 'name', 'direction' => request('sort') === 'name' && request('direction') === 'asc' ? 'desc' : 'asc'])) }}'">
                ...
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">MS Teams</th>
        ...
        <tbody class="bg-white divide-y divide-gray-200" id="talk915TableBody">
            @forelse($tutors as $tutor)
                ...
                <tr class="hover:bg-gray-50 talk915-row" data-searchable="{{ strtolower(($tutor->full_name ?? '') . ' ' . ($tutor->email ?? '') . ' ' . ($tutor->phone_number ?? '')) }}">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {{ $tutor->username ?? 'N/A' }}@talk915.com
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
